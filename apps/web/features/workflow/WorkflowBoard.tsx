...
import { DragEvent, FormEvent, type MouseEvent, useEffect, useMemo, useRef, useState } from 'react'
...
export function WorkflowBoard({ workflowSlug, basePath }: Readonly<{ workflowSlug: string; basePath: string }>) {
  const firm = useFirmContext(); const router = useRouter(); const search = useSearchParams(); const searchKey = search.toString(); const filters = filtersFromSearch(search); const { t } = useLanguage()
  ...
  const [dragged, setDragged] = useState<WorkItem | null>(null); const [message, setMessage] = useState(''); const [newStageId, setNewStageId] = useState<string | null>(null); const [expandedStageIds, setExpandedStageIds] = useState<string[]>([]); const [returnMove, setReturnMove] = useState<{ item: WorkItem; targetStageId: string } | null>(null)
  const pendingPanelOpen = useRef<ReturnType<typeof setTimeout> | undefined>(undefined)
  const clearPendingPanelOpen = () => {
    if (pendingPanelOpen.current === undefined) return
    clearTimeout(pendingPanelOpen.current)
    pendingPanelOpen.current = undefined
  }
  useEffect(() => () => {
  ...
  const scheduleTaskPanel = (item: WorkItem) => {
    clearPendingPanelOpen()
    pendingPanelOpen.current = setTimeout(() => {
      pendingPanelOpen.current = undefined
      openTask(item)
    }, 400)
  }
  const openTaskWorkspace = (item: WorkItem) => {
    clearPendingPanelOpen()
    openTask(item, true)
  }
  const activateTask = (event: MouseEvent<HTMLButtonElement>, item: WorkItem) => {
    if (event.detail === 0) {
      clearPendingPanelOpen()
      openTask(item)
      return
    }
    scheduleTaskPanel(item)
  }
  const persistMove = async (item: WorkItem, targetStageId: string, reviewNote?: string) => { if (item.stageId === targetStageId) return; const from = source.stages.findIndex((stage) => stage.id === item.stageId); const to = source.stages.findIndex((stage) => stage.id === targetStageId); if (!reviewNote && source.stages[from]?.attention === 'AWAITING_REVIEW' && to === from - 1) { setReturnMove({ item, targetStageId }); return }; setMessage(''); try { await operations.move(item, targetStageId, reviewNote); setMessage(`${item.title} ${t('workflow.moved')}`) } catch (error) { setMessage(error instanceof Error && error.message.startsWith('This work item was changed') ? t('workflow.moveConflict') : t('workflow.moveError')) } finally { setDragged(null) } }
  ...
  return <section className={styles.workspace}>
    ...
    <div className={taskReference && task.data ? styles.boardAndPanel : undefined}><div className={styles.board} aria-label={`${source.name} ${t('navigation.workflow').toLowerCase()}`}><p className="srOnly">{t('workflow.dragHelp')}</p>{stages.map((stage, index) => {
      const isExpanded = !isMobile || expandedStageIds.includes(stage.id)
      const stageContentId = `workflow-stage-${stage.id}`
      return <section key={stage.id} className={styles.column} data-attention={stage.attention} aria-label={`${stage.name} ${t('workflow.stage').toLowerCase()}`} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (dragged) void persistMove(dragged, stage.id) }}><div className={styles.columnTitle}><span className={styles.stageMarker} aria-hidden="true">{attentionMark(stage.attention)}</span><h2>{stage.name}</h2><span className={styles.stageCount}>{stage.items.length}</span></div><button type="button" className={styles.mobileStageToggle} aria-label={`${t('workflow.toggleStage')} ${stage.name}`} aria-expanded={isExpanded} aria-controls={stageContentId} onClick={() => setExpandedStageIds((current) => current.includes(stage.id) ? current.filter((id) => id !== stage.id) : [...current, stage.id])}><span className={styles.stageMarker} aria-hidden="true">{attentionMark(stage.attention)}</span><span>{stage.name}</span><span className={styles.stageCount}>{stage.items.length}</span></button><div id={stageContentId} hidden={isMobile && !isExpanded} className={isExpanded ? styles.stageContent : `${styles.stageContent} ${styles.stageContentCollapsed}`}>{stage.items.map((item) => <article key={item.id} className={styles.card} draggable={!operations.isSaving} onDragStart={(event: DragEvent<HTMLElement>) => { setDragged(item); event.dataTransfer.setData('text/forgeboard-item', item.id) }} onDragEnd={() => setDragged(null)}><button type="button" className={styles.cardActivation} aria-label={`${t('common.open')} ${item.title} ${t('common.details')}`} onClick={(event) => activateTask(event, item)} onDoubleClick={() => openTaskWorkspace(item)}><p className={styles.cardReference}>{item.taskReference}</p><h3>{item.title}</h3><p className={styles.assignment}>{t('workflow.assigned')} {item.ownerDisplayName ?? t('common.unassigned')}</p><div className={styles.cardBadges}><span data-due={dueState(item.dueDate)}>{item.dueDate ? `${t('common.due')} ${item.dueDate}` : t('common.noDeadline')}</span><span>{item.priority.toLowerCase()}</span></div></button><div className={styles.cardActions}><button type="button" aria-label={`${t('workflow.moveLeft')} ${item.title}`} disabled={index === 0 || operations.isSaving} onClick={(event) => { event.stopPropagation(); void persistMove(item, source.stages[index - 1].id) }}>←</button><button type="button" aria-label={`${t('common.open')} ${item.title} ${t('workflow.taskWorkspace').toLowerCase()}`} onClick={(event) => { event.stopPropagation(); openTaskWorkspace(item) }}>{t('workflow.openTaskWorkspace')}</button><button type="button" aria-label={`${t('workflow.moveRight')} ${item.title}`} disabled={index === source.stages.length - 1 || operations.isSaving} onClick={(event) => { event.stopPropagation(); void persistMove(item, source.stages[index + 1].id) }}>→</button></div></article>)}{firm.role !== 'READ_ONLY' && (newStageId === stage.id ? <form className={styles.newItemForm} aria-label={t('workflow.newWorkItem')} onSubmit={(event) => void create(event)}><label>{t('common.client')}<select name="clientId" required defaultValue=""><option value="" disabled>{t('workflow.selectClient')}</option>{activeClients.map((client) => <option key={client.id} value={client.id}>{client.displayName}</option>)}</select></label><label>{t('workflow.title')}<input name="title" required maxLength={200} /></label><label>Description<textarea name="description" maxLength={10000} /></label><label>{t('common.dueDate')}<input name="dueDate" type="date" /></label><label>{t('common.priority')}<select name="priority" defaultValue="NORMAL"><option>LOW</option><option>NORMAL</option><option>HIGH</option><option>URGENT</option></select></label><div><button className={styles.primaryButton} disabled={operations.isSaving || activeClients.length === 0}>{operations.isSaving ? t('workflow.creating') : t('workflow.createWorkItem')}</button><button type="button" className={styles.quietButton} onClick={() => setNewStageId(null)}>{t('common.cancel')}</button></div></form> : <button type="button" className={styles.addItemButton} aria-label={`${t('workflow.addWorkItemTo')} ${stage.name}`} onClick={() => setNewStageId(stage.id)}>{t('workflow.addWorkItem')}</button>)}{stage.items.length === 0 && <p className={styles.emptyStage}>{t('workflow.noWorkItemsInStage')}</p>}</div></section>
    })}</div>{taskReference && task.data && <TaskPanel detail={task.data} boardPath={boardPathWithoutTask(basePath, search)} taskPath={taskWorkspacePath(basePath, taskReference)} />}</div>
    {returnMove && <ReviewReturnDialog onCancel={() => { setReturnMove(null); setDragged(null) }} onConfirm={(note) => { const move = returnMove; setReturnMove(null); void persistMove(move.item, move.targetStageId, note) }} />}
