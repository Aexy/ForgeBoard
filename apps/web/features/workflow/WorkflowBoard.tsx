...
export const workflowStageCollisionDetection: CollisionDetection = (args) => {
  if (!args.pointerCoordinates) return closestCorners(args)

  const { x, y } = args.pointerCoordinates
  const containingStages = args.droppableContainers.filter((container) => {
    const rect = args.droppableRects.get(container.id)
    return rect && x >= rect.left && x <= rect.right && y >= rect.top && y <= rect.bottom
  })

  return containingStages.length
    ? closestCorners({ ...args, droppableContainers: containingStages })
    : []
}
...
export function WorkflowBoard({ workflowSlug, basePath }: Readonly<{ workflowSlug: string; basePath: string }>) {
  ...
  return <section className={styles.workspace}>
    ...
    })}</div><DragOverlay className={styles.dragOverlay} dropAnimation={null} transition="none">{activeItem ? <WorkflowDragOverlayCard item={activeItem} /> : null}</DragOverlay></DndContext>{taskReference && task.data && <TaskPanel detail={task.data} boardPath={boardPathWithoutTask(basePath, search)} taskPath={taskWorkspacePath(basePath, taskReference)} />}</div>
