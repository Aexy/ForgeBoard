...
export function WorkflowBoard({ workflowSlug, basePath }: Readonly<{ workflowSlug: string; basePath: string }>) {
  ...
  const boardPathWithoutTask = () => { const query = new URLSearchParams(search.toString()); query.delete('task'); return `${basePath}${query.size ? `?${query}` : ''}` }
  const openTask = (item: WorkItem, workspace = false) => { if (workspace) router.push(`${basePath}/tasks/${item.taskReference}`); else { const query = new URLSearchParams(search.toString()); query.set('task', item.taskReference); router.push(`${basePath}?${query}`) } }
  ...
  return <section className={styles.workspace}>
    ...
    <div className={taskReference && task.data ? styles.boardAndPanel : undefined}>
      ...
      {taskReference && task.data && <TaskPanel detail={task.data} boardPath={boardPathWithoutTask()} taskPath={`${basePath}/tasks/${taskReference}`} />}
    </div>
