...
export function Engagements() {
  ...
  return <section className={styles.workspace}>
    ...
    <div className={styles.grid}><section className={styles.section}><div><h2>{t('engagements.active')}</h2><p>{engagements.data?.length ?? 0} {t('engagements.scheduledCycles')}</p></div>{loading ? <p aria-live="polite">{t('engagements.loading')}</p> : engagements.data?.length === 0 ? <div className={styles.empty}><h3>{t('engagements.emptyTitle')}</h3><p>{t('engagements.emptyDescription')}</p></div> : <div className={styles.list}>{engagements.data?.map((engagement: Engagement) => <article className={styles.row} key={engagement.id}><div><span>{t(`engagements.status${engagement.status.split('_').map((word) => word[0] + word.slice(1).toLowerCase()).join('')}` as 'engagements.statusActive' | 'engagements.statusBlocked' | 'engagements.statusAwaitingReview' | 'engagements.statusComplete' | 'engagements.statusCancelled' | 'engagements.statusArchived')}</span><h3>{templateNames.get(engagement.templateId) ?? t('engagements.title')}</h3><p>{clientNames.get(engagement.clientId) ?? t('common.unknownClient')} · {displayDate(engagement.periodStart, t('common.noDeadline'))} to {displayDate(engagement.periodEnd, t('common.noDeadline'))}</p>{engagement.workItemId && <p>{t('engagements.boardWorkItemCreated')}</p>}</div><div><time dateTime={engagement.dueDate}>{t('common.due')} {displayDate(engagement.dueDate, t('common.noDeadline'))}</time><EngagementLifecycleControls engagement={engagement} /></div></article>)}</div>}</section>
