...
describe('WorkflowBoard', () => {
  ...
  it('opens query-backed detail after the single-click delay', () => {
    vi.useFakeTimers()
    render(<WorkflowBoard workflowSlug="monthly-close" basePath="/firms/hearth/workflow/monthly-close" />)
    fireEvent.click(screen.getByRole('button', { name: 'Open July close details' }), { detail: 1 })
    expect(router.push).not.toHaveBeenCalled()
    vi.advanceTimersByTime(400)
    expect(router.push).toHaveBeenLastCalledWith(`/firms/hearth/workflow/monthly-close?task=${item.taskReference}`)
  })
  it('opens only the full task route on a card double-click', () => {
    vi.useFakeTimers()
    render(<WorkflowBoard workflowSlug="monthly-close" basePath="/firms/hearth/workflow/monthly-close" />)
    ...
    expect(router.push).toHaveBeenLastCalledWith(`/firms/hearth/workflow/monthly-close/tasks/${item.taskReference}`)
  })
  ...
  it('opens query-backed detail immediately for keyboard activation', () => {
    render(<WorkflowBoard workflowSlug="monthly-close" basePath="/firms/hearth/workflow/monthly-close" />)
    fireEvent.click(screen.getByRole('button', { name: 'Open July close details' }), { detail: 0 })
    expect(router.push).toHaveBeenLastCalledWith(`/firms/hearth/workflow/monthly-close?task=${item.taskReference}`)
  })
  it('cancels a pending panel open when navigating through the explicit workspace action', () => {
    vi.useFakeTimers()
    render(<WorkflowBoard workflowSlug="monthly-close" basePath="/firms/hearth/workflow/monthly-close" />)
    fireEvent.click(screen.getByRole('button', { name: 'Open July close details' }), { detail: 1 })
    fireEvent.click(screen.getByRole('button', { name: 'Open July close task workspace' }))
    vi.advanceTimersByTime(400)
    expect(router.push).toHaveBeenCalledTimes(1)
    expect(router.push).toHaveBeenLastCalledWith(`/firms/hearth/workflow/monthly-close/tasks/${item.taskReference}`)
  })
  it('cancels a pending panel open when the board unmounts', () => {
    vi.useFakeTimers()
    const view = render(<WorkflowBoard workflowSlug="monthly-close" basePath="/firms/hearth/workflow/monthly-close" />)
    fireEvent.click(screen.getByRole('button', { name: 'Open July close details' }), { detail: 1 })
    view.unmount()
    vi.advanceTimersByTime(400)
    expect(router.push).not.toHaveBeenCalled()
  })
  it('cancels a pending panel open when board search state changes', () => {
    vi.useFakeTimers()
    const view = render(<WorkflowBoard workflowSlug="monthly-close" basePath="/firms/hearth/workflow/monthly-close" />)
    fireEvent.click(screen.getByRole('button', { name: 'Open July close details' }), { detail: 1 })
    mocks.searchParams.mockReturnValue(new URLSearchParams('priority=URGENT'))
    view.rerender(<LanguageProvider initialLanguage="en"><WorkflowBoard workflowSlug="monthly-close" basePath="/firms/hearth/workflow/monthly-close" /></LanguageProvider>)
    vi.advanceTimersByTime(400)
    expect(router.push).not.toHaveBeenCalled()
  })
