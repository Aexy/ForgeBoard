...
test('uses shareable workflow routes, task workspace, moves, and saved views', async ({ page, request }) => {
  ...
  await page.getByRole('group', { name: 'Language' }).last().getByRole('button', { name: 'Deutsch' }).click()
  await expect(page.locator('html')).toHaveAttribute('lang', 'de')
  await expect(page.getByRole('link', { name: 'Meine Aufgaben' })).toBeVisible()
  await page.getByRole('button', { name: `Verknüpfen ${documentRequestData.label}` }).click()
  await expect(page.getByRole('button', { name: `Verknüpfung lösen ${documentRequestData.label}` })).toBeVisible()
  await page.getByRole('button', { name: `Verknüpfung lösen ${documentRequestData.label}` }).click()
  await page.getByRole('group', { name: 'Sprache' }).last().getByRole('button', { name: 'English' }).click()
