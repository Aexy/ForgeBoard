...
import { createWorkflow, createWorkItem, getWorkflow, listWorkflows, moveWorkItem, WorkflowBoard } from './api/workflows'

export function WorkflowView({ firmId }: { firmId: string }) {
  const [workflows, setWorkflows] = useState<{ id: string; name: string }[]>([]); const [board, setBoard] = useState<WorkflowBoard | null>(null); const [clients, setClients] = useState<Client[]>([]); const [activity, setActivity] = useState<Activity[]>([]); const [error, setError] = useState(''); const [creatingBoard, setCreatingBoard] = useState(false); const [addingTo, setAddingTo] = useState<string | null>(null)
  useEffect(() => { Promise.all([listWorkflows(firmId), listClients(firmId)]).then(async ([found, clientList]) => { setWorkflows(found); setClients(clientList.filter((client) => client.status === 'ACTIVE')); if (found[0]) setBoard(await getWorkflow(firmId, found[0].id)) }).catch(() => setError('The workflow could not be loaded.')) }, [firmId])
  useEffect(() => { refreshActivity() }, [firmId])
  function refreshActivity() { listActivity(firmId).then(setActivity).catch(() => setError('Recent activity could not be loaded.')) }
  async function makeBoard(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); try { const created = await createWorkflow(firmId, { name: String(data.get('name')), stages: String(data.get('stages')).split(',').map((stage) => stage.trim()).filter(Boolean) }); setBoard(created); setWorkflows((all) => [...all, { id: created.id, name: created.name }]); setCreatingBoard(false); form.reset(); refreshActivity() } catch { setError('The workflow could not be created. Use at least two stages.') } }
  async function addItem(event: FormEvent<HTMLFormElement>, stageId: string) { event.preventDefault(); if (!board) return; const form = event.currentTarget; const data = new FormData(form); try { const item = await createWorkItem(firmId, board.id, { clientId: String(data.get('clientId')), stageId, title: String(data.get('title')), description: String(data.get('description')), dueDate: String(data.get('dueDate')) || null, priority: String(data.get('priority')) as 'LOW' | 'NORMAL' | 'HIGH' | 'URGENT' }); setBoard({ ...board, stages: board.stages.map((stage) => stage.id === stageId ? { ...stage, items: [...stage.items, item] } : stage) }); setAddingTo(null); refreshActivity() } catch { setError('The work item could not be created.') } }
  async function move(itemId: string, stageIndex: number, direction: -1 | 1) { if (!board) return; const target = board.stages[stageIndex + direction]; if (!target) return; try { await moveWorkItem(firmId, board.id, itemId, target.id); setBoard(await getWorkflow(firmId, board.id)); refreshActivity() } catch { setError('The work item could not be moved.') } }
  async function drop(itemId: string, targetStageId: string) { if (!board || board.stages.some((stage) => stage.id === targetStageId && stage.items.some((item) => item.id === itemId))) return; try { await moveWorkItem(firmId, board.id, itemId, targetStageId); setBoard(await getWorkflow(firmId, board.id)); refreshActivity() } catch { setError('The work item could not be moved.') } }
  return <section className="workspace"><header><div><p className="eyebrow">Client work</p><h1>{board?.name ?? 'Workflows'}</h1><p>Track every engagement from client request through review.</p></div><button onClick={() => setCreatingBoard(!creatingBoard)}>{creatingBoard ? 'Cancel' : '+ New workflow'}</button></header>
