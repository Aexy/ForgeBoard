...
export function WorkflowFilters({ clients, employees, filters, onChange, onReset, savedViews, canManageViews, onApplyView, onSaveView, onDeleteView }: { clients: Client[]; employees: Employee[]; filters: BoardFilters; onChange: (filters: BoardFilters) => void; onReset: () => void; savedViews?: Array<{ id: string; name: string }>; canManageViews?: boolean; onApplyView?: (id: string) => void; onSaveView?: (name: string) => void; onDeleteView?: (id: string) => void }) {
  return <div className="workflow-filters" aria-label="Workflow filters">
    ...
    {savedViews && savedViews.length > 0 && <label>Saved view<select aria-label="Apply saved workflow view" defaultValue="" onChange={(event) => { if (event.target.value) onApplyView?.(event.target.value); event.currentTarget.value = '' }}><option value="">Apply saved view</option>{savedViews.map((view) => <option value={view.id} key={view.id}>{view.name}</option>)}</select></label>}
