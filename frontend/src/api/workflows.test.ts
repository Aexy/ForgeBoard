...
describe('workflow API', () => {
  ...
  it('persists a movement with CSRF', async () => { const fetchMock = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({ headerName: 'X-XSRF-TOKEN', token: 'csrf' }))).mockResolvedValueOnce(new Response(JSON.stringify({ id: 'item-1' }))); vi.stubGlobal('fetch', fetchMock); await moveWorkItem('firm-1', 'flow-1', 'item-1', 'review'); expect(fetchMock).toHaveBeenLastCalledWith('/api/workflows/flow-1/items/item-1/position', expect.objectContaining({ method: 'PATCH', headers: expect.objectContaining({ 'X-ForgeBoard-Firm': 'firm-1', 'X-XSRF-TOKEN': 'csrf' }) })) })
})
