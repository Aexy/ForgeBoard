export type EmployeeWorkItem = { id: string; title: string; workflowId: string; stageId: string; stageName: string; dueDate: string | null }
...
export async function getMyWorkDashboard(firmId: string): Promise<EmployeeDashboard> {
  const response = await fetch('/api/dashboard/my-work', { credentials: 'include', headers: { 'X-ForgeBoard-Firm': firmId } })
