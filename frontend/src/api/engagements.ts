...
export const createEngagementTemplate = (firmId: string, details: Omit<EngagementTemplate, 'id' | 'version'>) => mutate<EngagementTemplate>(firmId, '/api/engagements/templates', 'POST', details)
export const createEngagementInstance = (firmId: string, templateId: string, details: { clientId: string; periodStart: string }) => mutate<Engagement>(firmId, `/api/engagements/templates/${templateId}/instances`, 'POST', details)
