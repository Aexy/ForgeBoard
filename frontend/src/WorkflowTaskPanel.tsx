...
export function WorkflowTaskPanel({ detail, onClose, onOpenWorkspace, mobile = false }: { detail: WorkItemDetail; onClose: () => void; onOpenWorkspace: () => void; mobile?: boolean }) {
  return <aside className={`task-panel${mobile ? ' task-sheet' : ''}`} role={mobile ? 'dialog' : undefined} aria-modal={mobile || undefined} aria-label={`${detail.item.title} details`}>
    ...
    <button type="button" className="primary-action" onClick={onOpenWorkspace}>Open task workspace</button>
  </aside>
}
